...
/**
 * Build final section list based on config, intelligence, and buyer awareness
 */
export function buildSectionList(
  config: IndustryLayoutConfig,
  intelligence: ExtractedIntelligence,
  ...
  const sections = new Set<SectionType>(config.sections.required);

  // 2. Evaluate and add conditional sections
  const conditionalWithScores = config.sections.conditional
    .filter((conditional) => evaluateCondition(conditional.condition, intelligence))
    .sort((a, b) => b.priority - a.priority);

  for (const conditional of conditionalWithScores) {
    sections.add(conditional.section);
  }

  // 3. Get awareness-appropriate ordering
  const orderedSections = config.sectionOrder[buyerAwareness] || config.sectionOrder.problemAware;

  // 4. Reorder sections to match awareness order
  const sectionsArray = Array.from(sections);
  const orderedResult = orderedSections.filter((section) => sectionsArray.includes(section));

  // 5. Append any sections not in awareness order
  const unordered = sectionsArray.filter((s) => !orderedSections.includes(s));
  const combined = [...orderedResult, ...unordered];

  // 6. Remove avoided sections
  const cleaned = combined.filter((section) => !config.sections.avoid.includes(section));

  // 7. Respect maxSections limit
  const limited = cleaned.slice(0, config.sections.maxSections);
