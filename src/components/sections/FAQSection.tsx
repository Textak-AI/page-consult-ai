...
// Healthcare FAQ Item component
interface HealthcareFAQItemProps {
  item: FAQItem;
  index: number;
  isEditing: boolean;
  isExpanded: boolean;
...
const HealthcareFAQItem = memo(function HealthcareFAQItem({
  item,
  index,
  isEditing,
  isExpanded,
  ...
  return (
    <motion.div
      ...
      className="relative"
    >
      <div className="bg-teal-50 rounded-xl overflow-hidden flex border border-teal-100">
        {isEditing && (
          <div className="flex items-center px-2 cursor-grab active:cursor-grabbing text-teal-400 hover:text-teal-600">
            <GripVertical className="w-4 h-4" strokeWidth={1.5} />
          </div>
        )}
        ...
        <div className="flex-1">
          <button
            className="w-full p-6 text-left flex items-center justify-between hover:bg-teal-100/50 transition-colors"
            onClick={onToggle}
          >
            {isEditingThis ? (
              <Input
                ...
                onChange={(e) => setLocalQuestion(e.target.value)}
                onBlur={handleQuestionBlur}
                className="flex-1 mr-4 bg-white border-teal-300"
                autoFocus
                onClick={(e) => e.stopPropagation()}
              />
            ) : (
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.2 }}>
                <ChevronDown className="w-5 h-5 text-teal-500" strokeWidth={1.5} />
              </motion.div>
            </div>
          </button>
                ...
                <div className="px-6 pb-6">
                  {isEditingThis ? (
                    <Textarea
                      ...
                      onChange={(e) => setLocalAnswer(e.target.value)}
                      onBlur={handleAnswerBlur}
                      className="w-full min-h-[100px] bg-white border-teal-300"
                      onClick={(e) => e.stopPropagation()}
                    />
                  ) : (
                    <p className="text-slate-600 leading-relaxed" itemProp="text">
          ...
          {isEditing && (
            <div className="mt-6 flex justify-center">
              <Button
                ...
                className="gap-2 border-dashed border-teal-300 text-teal-600 hover:border-teal-400 hover:text-teal-700 hover:bg-teal-50/50"
              >
